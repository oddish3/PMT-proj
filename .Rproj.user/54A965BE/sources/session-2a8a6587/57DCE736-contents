...
run_regression <- function(country_name, dep_var, model_type = "ols") {
  ...
  # Run regression
  if (model_type == "ols") {
    model <- feols(formula, data = country_data, vcov = ~EA)
    predictions <- predict(model)
    var_name <- paste0("yhat_pmt_sh_", dep_var)
    data[[var_name]][data$country == country_name] <<- predictions
  } else if (model_type == "probit") {
    model <- glm(formula, family = binomial(link = "probit"), data = country_data)
    predictions <- predict(model, type = "response")
    var_name <- paste0("yhat_pmt_sh_pr_", dep_var)
    data[[var_name]][data$country == country_name] <<- predictions
  }
...
for (dep_var in depvars) {
  for (country in countries) {
    cat("\nDebug: Processing", country, "for", dep_var, "\n")
